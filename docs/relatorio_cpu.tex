...
\usepackage{lmodern}
% Ajustado geometry para incluir footskip apropriado, evitando que o texto invada o rodapé
...
\begin{titlepage}
  ...
  \vspace{0.4cm}
  {\large Projeto e implementação de uma CPU RISC-V de 32 bits\\[0.2em]
  com pipeline de 5 estágios em VHDL\\[0.2em]
  com forwarding, detecção de hazards e flush de controle}

  \vspace{3.0cm}
  \begin{tabular}{rl}
    \textbf{Alunos:}      & Luiz Cavalini, Eduardo Viana, Henrique Kezen \\
                          & e Rafael Maurício \\[0.5em]
    \textbf{Repositório:} & \url{https://github.com/LuizCavalini/AC-CPU-RV32I} \\[0.5em]
...
Este relatório descreve o projeto e a implementação de uma CPU RISC-V de
32 bits (RV32I) com pipeline de 5 estágios em VHDL. O objetivo é construir
um processador funcional que execute o subconjunto de instruções inteiras
...
\begin{table}[H]
  \centering
  \caption{Instruções implementadas por categoria}
  \small\rowcolors{2}{rowalt}{white}
  \begin{tabular}{ll}
    \toprule
    \rowcolor{headerblue}
    \color{white}\textbf{Categoria} & \color{white}\textbf{Instruções} \\
    \midrule
    Aritmética   & \code{add, addi, auipc, sub} \\
    Lógica       & \code{and, andi, or, ori, xor, xori} \\
    Deslocamento     & \code{sll, slli, srl, srli} \\
    Memória      & \code{lw, lui, sw} \\
...
\subsection{Visão geral dos 5 estágios}

A CPU é organizada em cinco estágios clássicos separados por registradores
de pipeline:

\begin{enumerate}[leftmargin=2em]
  \item \textbf{IF (Instruction Fetch):} O PC endereça a memória de
    instruções e a instrução é carregada no registrador IF/ID.
  \item \textbf{ID (Instruction Decode):} A instrução é decodificada,
    os operandos são lidos do banco de registradores e o imediato é
    gerado por extensão de sinal.
  \item \textbf{EX (Execute):} A ALU executa a operação. A Forwarding Unit
    seleciona os operandos corretos. Branches e jumps calculam seus alvos.
  \item \textbf{MEM (Memory Access):} \code{lw} lê e \code{sw} escreve na
    memória de dados.
  \item \textbf{WB (Write Back):} O resultado final é escrito no banco de
    registradores.
\end{enumerate}
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Pipeline de 5 estágios com forwarding e detecção de hazards}
  \label{fig:pipeline}
\end{figure}
...
A ALU de 32 bits recebe dois operandos e um seletor de operação de 4 bits:

\begin{table}[H]
  \centering
  \caption{Operações da ALU}
  \small\rowcolors{2}{rowalt}{white}
  \begin{tabular}{cll}
    \toprule
    \rowcolor{headerblue}
    \color{white}\textbf{alu\_ctrl} & \color{white}\textbf{Operação} &
    ...
    \code{1000} & SLT & comparação com sinal \\
    \bottomrule
  \end{tabular}
\end{table}

A flag \code{zero\_o} é ativada quando o resultado é zero, usada por
...
Armazena os 32 registradores de 32 bits. O registrador \reg{0} é fixo em zero:
escritas são ignoradas e leituras sempre retornam \code{0x00000000}. A escrita
é síncrona e a leitura é assíncrona (combinacional), permitindo que WB escreva
...
Suporta os seis formatos RV32I (R, I, S, B, U, J) com extensão de sinal do
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \code{alu\_ctrl} & 4 & seleciona operação da ALU \\
    \code{mem\_we}   & 1 & habilita escrita na memória (\code{sw}) \\
    \code{mem\_re}   & 1 & habilita leitura da memória (\code{lw}) \\
    ...
    \code{imm}       & 32 & imediato com extensão de sinal \\
    \bottomrule
  \end{tabular}
\end{table}
...
trânsito diretamente para as entradas da ALU:
...
Codificação dos muxes: \code{"00"} = sem forwarding, \code{"01"} = MEM/WB,
\code{"10"} = EX/MEM.

\subsection{Load-Use Hazard --- Stall}

Quando \code{lw} é seguido imediatamente por instrução que usa o valor
...
A unidade mantém PC e IF/ID inalterados e insere NOP no ID/EX.

\subsection{Control Hazards --- Flush}

Branches e jumps são resolvidos no estágio EX. Quando tomados, o pipeline:
(1) redireciona o PC para o alvo, e (2) descarta as duas instruções incorretas
já buscadas (flush do IF/ID e ID/EX). Custo fixo: 2 ciclos por branch tomado.

% ============================================================
\section{Simulação e Verificação}

\subsection{Testbenches individuais}

\begin{table}[H]
  \centering
  \caption{Resultado dos testbenches por módulo}
  \small\rowcolors{2}{rowalt}{white}
  \begin{tabular}{lcc}
    \toprule
    \rowcolor{headerblue}
    ...
    \code{cpu\_single\_cycle} & programa 13 instruções & PC final = 0x44 \\
    \code{cpu\_pipeline}      & programa 12 instruções & PC final = 0x5C \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Componente \code{top}:} wrapper VHDL que instancia a
    \code{cpu\_pipeline} e exporta sinais de depuração adicionais.
  \item \textbf{ROM (256 palavras $\times$ 32 bits):} carregada com o programa
    de teste via arquivo \code{program.hex}.
  \item \textbf{RAM (256 palavras $\times$ 32 bits):} memória de dados com
    portas separadas de leitura e escrita.
  \item \textbf{Distribuidores:} extraem os bits {[}9:2{]} dos endereços de
    32 bits para endereçar as memórias por palavra.
\end{itemize}

Os seguintes sinais de depuração são visíveis durante a simulação:

\begin{table}[H]
  \centering
  \caption{Sinais de depuração exportados pelo componente \code{top}}
  \small\rowcolors{2}{rowalt}{white}
  \begin{tabular}{lll}
    \toprule
    \rowcolor{headerblue}
    ...
    \color{white}\textbf{Descrição} \\
    \midrule
    \code{pc\_debug\_o}    & 32 & PC atual (endereço da instrução em IF) \\
    \code{instr\_debug\_o} & 32 & Instrução sendo decodificada \\
    \code{alu\_debug\_o}   & 32 & Endereço de memória (resultado ALU no MEM) \\
    \code{stall\_debug\_o} & 1  & Indica stall ativo (PC não avança) \\
    \code{flush\_debug\_o} & 1  & Indica flush ativo (descarte de instrução após desvio tomado) \\
    \code{imem\_addr\_o}   & 32 & Endereço da memória de instruções \\
    \code{dmem\_addr\_o}   & 32 & Endereço da memória de dados \\
    \code{dmem\_wdata\_o}  & 32 & Dado a escrever na memória (\code{sw}) \\
    \code{dmem\_we\_o}     & 1  & Habilitação de escrita na memória \\
    \code{dmem\_re\_o}     & 1  & Habilitação de leitura da memória \\
    \bottomrule
  \end{tabular}
\end{table}

A simulação interativa no Digital confirmou os seguintes comportamentos:
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{Xl}
    ...
    Leitura (\code{lw}): \code{x11 = 13} (valor lido de volta) & Inferido\footnotemark\\
...
\begin{figure}[H]
   \centering
   \includegraphics[width=0.85\textwidth]{digital.png}
...
Durante a validação interativa no Digital, observou-se que o sinal
\code{flush\_debug\_o} nunca era ativado durante um branch tomado, apesar do
PC ser redirecionado corretamente. Investigação do código-fonte revelou
dois problemas:

\begin{enumerate}[leftmargin=2em]
  \item O pino \code{flush\_debug\_o} no wrapper \code{top.vhd} estava
    conectado a uma heurística derivada de sinais de barramento externos
    (\code{dmem\_we\_o or dmem\_re\_o}), sem relação com o flush real do
    pipeline.
  \item Mais grave: o sinal \code{flush\_idex\_s}, que controla o descarte
    do registrador ID/EX, era acionado apenas pela detecção de load-use
    ...
    limpo, a instrução do caminho errado que já havia avançado para o
    ID/EX no mesmo ciclo do redirecionamento escapava do descarte e
    seria executada indevidamente.
\end{enumerate}

A correção consistiu em: (1) exportar sinais de depuração reais
...
as heurísticas do wrapper; e (2) alterar a lógica de flush do ID/EX para 
\code{flush\_idex\_s <= flush\_idex\_haz\_s or pc\_redirect\_s}, garantindo o
descarte tanto em load-use hazard quanto em desvio tomado.

A correção foi validada por um teste dirigido: instruções
"veneno" (\code{addi} com valores identificáveis) foram inseridas nos
slots de atraso do \code{beq} e do \code{jalr}.
Após a correção,
\code{alu\_debug\_o} nunca exibiu esses valores em nenhum ciclo, confirmando
que as instruções de caminho errado são corretamente descartadas.
Adicionalmente, os testbenches \code{cpu\_pipeline\_tb} e \code{reset\_tb} do
GHDL foram reexecutados após a correção (PC final = 0x5C e reset para
0x04, respectivamente), confirmando ausência de regressão.


\section{Premissas e Decisões de Projeto (Parte 1)}

\begin{enumerate}[leftmargin=2em]
  \item \textbf{Branch resolvido no estágio EX.} Reutiliza a ALU (subtração
  ...
  \item \textbf{Forwarding cobre todos os casos exceto load-use.} EX/MEM$\to$EX
    e MEM/WB$\to$EX cobrem dependências RAW de 1 e 2 ciclos. Load-use exige
    1 stall obrigatório.

  \item \textbf{Memórias separadas (Harvard).} Conforme o enunciado, memórias
    de instrução e dados distintas, cada uma entregando uma palavra por ciclo.

  \item \textbf{Reset síncrono.} Todos os registradores e o PC respondem ao
    \code{rst\_i} na borda de subida, garantindo inicialização determinística.

  \item \textbf{Estados internos exportados.} Sinais internos relevantes (flags
    zero, sinais de forwarding, stall) são expostos como portas para depuração
    no Digital, conforme exigido pelo enunciado.

  \item \textbf{AUIPC via mux no operando A.} Quando opcode = \code{0010111},
    o operando A da ALU é PC em vez de rs1, calculando PC + imm sem
    hardware adicional.

  \item \textbf{Espaço de endereçamento limitado a 256 palavras.} ROM e RAM
    são endereçadas pelos bits [9:2] do barramento de 32 bits (descartando
    os 2 LSBs, por alinhamento de palavra, e os bits superiores, fixados em
    zero), resultando em 1KB de espaço endereçável para cada memória. Essa
    simplificação é adequada ao escopo de um programa de teste da
    disciplina e não compromete a arquitetura da CPU, que opera sobre o
    barramento de endereço completo de 32 bits.

  \item \textbf{CPU inativa durante carga de dados.} Como todos os elementos
    de estado da CPU (PC e registradores de pipeline) são atualizados
    exclusivamente na borda de subida de \code{clk\_i}, e a carga do programa
    na ROM/RAM ocorre de forma assíncrona pela interface do Digital sem
    geração de pulsos de clock, o estado interno da CPU permanece
    inalterado durante o carregamento de dados, atendendo ao requisito do
    enunciado sem necessidade de lógica adicional.
\end{enumerate}
...
\begin{figure}[H]
   \centering
   \includegraphics[width=0.85\textwidth]{digital_vetorial.png}
    \caption{Simulação da CPU vetorial no Digital — RAM com os 10 resultados confirmados}
    \label{fig:digital-vetorial}
\end{figure}
...
O projeto resultou em uma CPU RV32I funcional com pipeline de 5 estágios,
implementando as instruções obrigatórias. A validação em GHDL confirmou
o funcionamento correto do forwarding, detecção de load-use e flush de
...
A extensão vetorial (Parte 2) adicionou oito instruções customizadas
via opcode \code{custom-0}, reaproveitando o somador vetorial do
projeto AC-AVX-Adder e um novo deslocador de barril com a mesma
filosofia de contenção de lane. A integração revelou e corrigiu dois
bugs pré-existentes --- um hazard RAW de distância-3 no banco de
registradores e um erro de encadeamento de carry em subtrações
vetoriais multi-bloco --- ambos validados com regressão completa. As 8
instruções vetoriais foram confirmadas corretas tanto via GHDL (10/10
casos) quanto interativamente no Digital (10/10 valores na RAM),
incluindo os dois casos que expuseram os bugs corrigidos.
...
\begin{enumerate}[leftmargin=2em]
  \item Patterson, D. A. \& Hennessy, J. L. \textit{Computer Organization and
    Design: RISC-V Edition}, 2\textordfeminine~ed., Elsevier, 2021. Capítulos 4 e 5.

  \item RISC-V International. \textit{The RISC-V Instruction Set Manual,
    Volume I: Unprivileged ISA}, versão 20191213.
